...
function HistoryBookingTicket({ infoUser }) {
  const renderHistoryBookingTicket = () => {
    return infoUser?.thongTinDatVe?.map((_, i) => (
      <div key={i} className="xl:w-1/3 md:w-1/2 p-4 w-full">
        <div className="border border-gray-200 p-6 pt-10 rounded-lg">
    ...
    <div>
      <section className="text-gray-600 body-font">
        <div className="container px-1 lg:px-5 py-12 mx-auto">
          ...
          <div className="flex flex-wrap -m-4">{renderHistoryBookingTicket()}</div>
        </div>
      </section>
    </div>
  );
...
export default HistoryBookingTicket;
